...
ModuleServer = (function() {
   ...
   */
  ModuleServer.prototype._loadedPackages = null;
  ...
  function ModuleServer(router1, modulePath, configPath) {
    this.router = router1;
  ...
  ModuleServer.prototype.load = function(name, path) {
    var _pkg, err, i, len, main, origPath, paths, pkg, ref, updir;
    ref = this._loadedPackages;
    ...
    path = origPath = this.findPath(path);
    pkg = null;
    while (pkg === null) {
      try {
  ...
  ModuleServer.prototype.loadSubmodules = function(pkg, path) {
    ...
    for (name in pkgs) {
      ...
      if (subpath[0] === ".") {
        subpath = require("path").resolve(path, subpath);
      }
      results.push(this.load(name, subpath));
    }
    return results;
  };
  ...
  ModuleServer.prototype.middleware = function(name, path) {
    ...
    return this.moduleRouter.use("/" + name, router);
  };
  ...
  ModuleServer.prototype.finalize = function() {
    return this.router.use(this.modulePath, this.moduleRouter);
  };
  ...
  ModuleServer.prototype.correctPaths = function(name, paths) {
    ...
    for (script in paths) {
      ...
      paths[script] = location.replace("$PUBLIC", "" + this.modulePath + name).replace(/\$COFFEE\/(.*)\.coffee$/, "" + this.modulePath + name + "/$1.js").replace("$COFFEE", "" + this.modulePath + name);
    }
    return paths;
  };
