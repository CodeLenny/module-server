...
ModuleTest = (function() {
  ...
  ModuleTest.prototype.onit = function(name, handler, timeout, cb) {
    ...
    this._it.push((function(_this) {
      return function(router, started, itStarted) {
        var args;
    ...
    fn = (function(_this) {
      return function(i, count) {
        ...
        return describe(name, function() {
          var _it, _phantomStarted, _started, k, len, moduleServer, onItStart, onStart, path, phantomInstance, phantomPage, ref2, ref3, ref4, ref5, results, ret, router, server;
          ref2 = [], phantomInstance = ref2[0], phantomPage = ref2[1];
          router = _this._server ? _this._server() : express();
          if (Array.isArray(router)) {
            ref3 = router, router = ref3[0], moduleServer = ref3[1];
          }
          ...
          server = null;
          _started = 0;
          _phantomStarted = false;
          onStart = function() {
            return ++_started;
          };
          onItStart = function(_mocha) {
            _mocha.slow(300);
            if (_started !== _this._it.length || _phantomStarted) {
              return;
            }
            _mocha.slow(1250);
            return getPort(function(port) {
              server = router.listen(port);
              _phantomStarted = true;
          ...
          for (k = 0, len = ref5.length; k < len; k++) {
            _it = ref5[k];
            results.push(_it(router, onStart, onItStart));
          }
          return results;
        });
      };
